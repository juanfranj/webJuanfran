...
def iniciarRegistro(mesas_maximas, torneos):
    iniciarArchivo()
    registro = {"mesas":0, "registros":0}
    mesas = 0
    registros = buscarRegistros()
    fin = ConsultarFin()
    while not fin:
        registros["mesas"] = buscarMesas()
        
        if registros["mesas"] == 0 and registro["registros"] == 0:
            mesas += 1
            escribir_registro()
            actualizarRegistros(1)  
            registro["registros"] = buscarRegistros()       
             
        elif ((registro["mesas"] + registro["registros"]) <= mesas_maximas) and (registro["mesas"] != mesas):
            mesas += 1
            registro = escribir_registro(registro)
            registro -= 1
        fin = ConsultarFin()
        sleep(3)
...
def buscarMesas():
    try:
        path = "Z:/Bot/registroAutomatico/numeroMesas.txt"
        archivo = open(path, "r")
        mesas = archivo.readline()
        archivo.close()
    except:
        path = "C:/Capturador/Bot/registroAutomatico/numeroMesas.txt"
        archivo = open(path, "r")
        mesas = archivo.readline()
        archivo.close()
    return int(mesas)
...
def buscarRegistros():
    try:
        path = "Z:/Bot/registroAutomatico/numeroRegistros.txt"
        archivo = open(path, "r")
        registros = archivo.readline()
        archivo.close()
    except:
        path = "C:/Capturador/Bot/registroAutomatico/numeroRegistros.txt"
        archivo = open(path, "r")
        registros = archivo.readline()
        archivo.close()
...
def iniciarArchivo():
    try:
        path = "Z:/Bot/registroAutomatico/RegistroAutomatico.txt"
        archivo = open(path, "w")
        archivo.write("Start")
        archivo.close()
    except:
        path = "C:/Capturador/Bot/registroAutomatico/RegistroAutomatico.txt"
        archivo = open(path, "w")
        archivo.write("False")
        archivo.close()

def FinalizarRegistro():
    try:
        path = "Z:/Bot/registroAutomatico/RegistroAutomatico.txt"
        archivo = open(path, "w")
        archivo.write("True")
...
def actualizarRegistros(actualizar):
    try:
        path = "Z:/Bot/registroAutomatico/numeroRegistros.txt"
        archivo = open(path, "r")
        registro = int(archivo.readline()) + actualizar
        archivo.close()
        archivo.open(path, "w")
